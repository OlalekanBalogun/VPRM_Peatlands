...
ARB_tower_obs.SIF <- read.csv("ARB-Tower-NARR-Obs-ready.csv", stringsAsFactors = FALSE)
...
ARB_tower_obs.SIF.dates <- t(as.data.frame(strsplit(ARB_tower_obs.SIF[["Date"]],' '))) #t is (matrix transpose)
row.names(ARB_tower_obs.SIF.dates) = NULL
ARB_tower_obs.SIF ["Date"] <- chron(dates=ARB_tower_obs.SIF.dates[,1],times=ARB_tower_obs.SIF.dates[,2], format = c(dates = "m/d/y", times = "h:m:s"))
class(ARB_tower_obs.SIF[["Date"]])
...
phen_filled.SIF <- interp_phenology(ARB_phen_rearranged.SIF, ARB_tower_obs.SIF[['Date']])
...
## determine the phenology phase for each tower observation date
#phen_filled.SIF <- interp_phenology(ARB_phen, ARB_tower_obs.SIF[['date']])
...
ARB_dd.SIF <- VPRM_driver_data(name_long="Attawapiskat ON",
                               ...
                               tower_date=ARB_tower_obs.SIF[['Date']],
                               NEE_obs=ARB_tower_obs.SIF[['FC']],
                               T=ARB_tower_obs.SIF[['TA']],
                               PAR=ARB_tower_obs.SIF[['PAR']],
...
##################################################Diagnosis#############################################
NEE.Diff.SIF <- data.frame(Date = ARB_dd.SIF[["data"]][["date"]], NEE_VPRM = ARB_dd.SIF[["data"]][["NEE_VPRM"]], NEE_obs = ARB_dd.SIF[["data"]][["NEE_obs"]], NEE_Residual = ARB_dd.SIF[["data"]][["NEE_VPRM"]] - ARB_dd.SIF[["data"]][["NEE_obs"]])
...
plot <- plot + xlab("Time") + ylab(expression(NEE~(mu*mol~m^{-2}~s^{-1}))) + ggtitle(paste("ARB Mean Diurnal NEE" ,"(June-October, 2011-2015)"))
...
plot <- plot + scale_x_discrete(breaks = c("01:00","04:00","07:00","10:00","13:00","16:00","19:00","22:00"))
...
plot <- plot + scale_y_continuous(breaks=seq(-4,2,1)) #Sets the axis range and interval ("by")
plot <- plot + coord_cartesian(ylim = c(-4, 2)) #Forces the limits to be set accordingly
#plot
#save graph to output as high resoultion, Windows compatible format .EMF
fileout <- paste("Line-ARB Mean Diurnal NEE","_LineCycle",".emf",sep="")
...
plot <- plot + scale_color_manual(values = c("#00BA38", "#619CFF")) #New line for ONLY-SIF plot to set appropriate colors##################
plot <- plot + xlab("Month") + ylab(expression(NEE~(mu*mol~m^{-2}~s^{-1}))) + ggtitle(paste("ARB Mean Monthly NEE" ,"(June-October, 2011-2015)"))
...
plot <- plot + theme(legend.position = "bottom")
plot <- plot + geom_hline(yintercept=0, color="black")
plot <- plot + scale_y_continuous(breaks=seq(-2.5,0.0,0.5)) #Sets the axis range and interval ("by")
plot <- plot + coord_cartesian(ylim = c(-2.5, 0.1)) #Forces the limits to be set accordingly
#plot
#save graph to output as high resoultion, Windows compatible format .EMF
fileout <- paste("Line-ARB Mean Monthly NEE","_LineCycle",".emf",sep="")
x11(width=6, height=4) #default dim is 7 #Open new device #2 (note: no #1 because #1 is null)
dev.cur()
plot
savePlot(fileout, "emf", device = dev.cur())
...
cat("\n"); 
VPRMstate = "Model Run and Data Analysis Completed for ARB!"
cat("\n"); 
cat(VPRMstate);cat("\n"); cat("\n")
