...
plot <- plot + xlab("Variables") + ylab(expression(CO[2]~Fluxes~(mu*mol~m^{-2}~s^{-1}))) + ggtitle(paste("ARB Mean Fluxes " ,"(June-October, 2000-2019)"))
...
plot <- plot + scale_y_continuous(breaks=seq(-2,3,1), expand = c(0, 0)) #Sets the axis range and interval ("by")
plot <- plot + coord_cartesian(ylim = c(-2, 3)) #Forces the limits to be set accordingly
#save graph to output as high resoultion, Windows compatible format .EMF
fileout <- paste("Meso-ARB Mean Fluxes","_Bar",".emf",sep="")
...
#file.move("Meso_CO2Fluxes_Summary_Stats.csv", "~/Ecosystem_Modelling/VPRM_POLAR/MODELRUN_FINAL/ARB_MESO/Stats")
...
# NEW LINES ENDS - MESO: CARBON FLUXES BOX/BAR --------------------------------------------------- ##########################################################
...
library(filesstrings)
file.move("Meso_Fluxes_Annual_Mean.csv", "~/Ecosystem_Modelling/VPRM_POLAR/MODELRUN_FINAL/ARB_MESO/Stats")
...
#Move the excel .csv files to the stats folder
library(filesstrings)
#file.move("Meso_Fluxes_Annual_gC.csv", "~/Ecosystem_Modelling/VPRM_POLAR/MODELRUN_FINAL/ARB_MESO/Stats")
...
#plot <- plot + scale_color_manual(values = c("#00BA38", "#619ARBF")) #New line for ONLY-SIF plot to set appropriate colors##################
plot <- plot + xlab("Year") + ylab(expression(Carbon~Fluxes~(g~C~m^{-2}))) + ggtitle(paste("ARB Annual Flux Anomalies for OFF & ON" ,"(June-October, 2000-2019)"))
...
#save graph to output as high resoultion, Windows compatible format .EMF
fileout <- paste("Bar-Meso-ARB Annual Flux gC Anomalies","_BarGraph",".emf",sep="")
#x11(width=7, height=4) #default dim is 7 #Open new device #2 (note: no #1 because #1 is null)
...
#file.move("Meso-ALL_June_Annual_Fluxes_Annual_anomaly_gC_Combined.csv", "~/Ecosystem_Modelling/VPRM_POLAR/MODELRUN_FINAL/ARB_MESO/Stats")
